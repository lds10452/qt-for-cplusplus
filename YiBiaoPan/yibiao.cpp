...
#include <qmath.h>
#include<QDebug>
yiBiao::yiBiao(QWidget *parent)
    : QWidget(parent)
{  m_updateTimer = new QTimer(this);
    m_updateTimer->setInterval(50);//间隔，微妙微单位，转动速度。

    m_startAngle = 30;  //0刻度的起始角度
    m_endAngle = 30;
...
void yiBiao ::drawCrown(QPainter *painter)   //绘制表冠
{
    painter->save();
    int radius = 100;
   ...
   lg1.setColorAt(1, Qt::magenta);   //只是粗略的颜色，具体的可以参考RGB颜色查询对照表
...
void yiBiao ::drawScaleNum(QPainter *painter)  //绘制刻度数字
{
    painter->save();
...
void yiBiao ::drawScale(QPainter *painter)  //绘制刻度线
{
    painter->save();
...
void yiBiao ::drawTitle(QPainter *painter)
{
    painter->save();
...
void yiBiao ::drawNumericValue(QPainter *painter)
{
...
void yiBiao ::drawIndicator(QPainter *painter)
{
    painter->save();
    QPolygon pts;
    pts.setPoints(3, -2,0, 2,0, 0,60);	/* (-2,0)/(2,0)/(0,60) *///第一个参数是 ，坐标的个数。后边的是坐标
    //pts.setPoints(4, -30,0, 30,0, 30,60,-30,60);
    painter->rotate(m_startAngle);
...
void yiBiao ::paintEvent(QPaintEvent *)
{
